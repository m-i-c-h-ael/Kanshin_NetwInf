...
if (testmodeAns== 'y'){
  testcase_fun= testcase19_dir
  testMatch= str_match('testcase19_dir','^(test)case([0-9]+.*)')
  testString= testMatch[3]
  gSol_exist= 'y'
...
plot_graph_overTime= function(all_t_graph_l,plotSuffix=plotSuffix,
  ...
  if(length(V(all_t_graph_l[[1]]))>0) {
    ...
    if (PC_ans== 'n'){
      # legend(0.9,-0.1, leg_TPs,pt.bg=leg_colors,  title='t/min',       #TP legend for PC_ans=='y
      ...
      legend(0.9,1.1, leg_TPs,pt.bg=leg_colors,  title='t/min',
...
for (PC_ans_idx in seq_along(PC_ans_vec)){
  ...
  V(all_t_graph_sG1_N3)$t= 
    V(all_t_graph_l2[[1]])$t[match(V(all_t_graph_sG1_N3)$name,V(all_t_graph_l2[[1]])$name)]
  V(all_t_graph_sG1_N3)$type= 
    V(all_t_graph_l2[[1]])$type[match(V(all_t_graph_sG1_N3)$name,V(all_t_graph_l2[[1]])$name)]
  V(all_t_graph_sG1_N3)$type= 
    V(all_t_graph_l2[[1]])$type[match(V(all_t_graph_sG1_N3)$name,V(all_t_graph_l2[[1]])$name)]
  V(all_t_graph_sG1_N3)$sigf= 
    V(all_t_graph_l2[[1]])$sigf[match(V(all_t_graph_sG1_N3)$name,V(all_t_graph_l2[[1]])$name)]
  V(all_t_graph_sG1_N3)$color= 
    V(all_t_graph_l2[[1]])$color[match(V(all_t_graph_sG1_N3)$name,V(all_t_graph_l2[[1]])$name)]
  V(all_t_graph_sG1_N3)$frameCol= 
    V(all_t_graph_l2[[1]])$frameCol[match(V(all_t_graph_sG1_N3)$name,V(all_t_graph_l2[[1]])$name)]
  V(all_t_graph_sG1_N3)$labCol= 
    V(all_t_graph_l2[[1]])$labCol[match(V(all_t_graph_sG1_N3)$name,V(all_t_graph_l2[[1]])$name)]
  if(length(V(all_t_graph_l2[[1]])$commCol)>0){
    V(all_t_graph_sG1_N3)$commCol= 
      V(all_t_graph_l2[[1]])$commCol[match(V(all_t_graph_sG1_N3)$name,V(all_t_graph_l2[[1]])$name)] 
  }
  ...
  if (PC_ans== 'n'){
    legend(0.9,-0.1,legend=leg_TPs,pt.bg=rgb(0.3,leg_TPs/60,0.6),  title='t/min',
           pch=21,col="#777777", pt.cex=2, cex=.8, bty="n", ncol=1)     #
    legend(0.3,0.9,c('other','increased betw. cent.'),  title='Node type',
           pch=1,col=c('black','red'), pt.cex=2, cex=.8, bty="n", ncol=1) 
  } else {
    legend(0.7,1.1,,legend=leg_TPs,pt.bg=rgb(0.3,leg_TPs/60,0.6),  title='t/min',
           pch=21,col="#777777", pt.cex=2, cex=.8, bty="n", ncol=1)     #
...
ggarrange(
  ggarrange(G1a,G2a,G3a,nrow=3,ncol=1,widths=c(3),heights=(2),legend="none"),
  ggarrange(G1b,G2b,G3b,nrow=3,ncol=1,widths=c(3),heights=(2),legend="none"),
